...
interface InsuranceDao {
    @Query("SELECT * FROM insurance ORDER BY expiryDate")
    fun observeAll(): Flow<List<InsuranceEntity>>
    @Query("SELECT * FROM insurance WHERE expiryDate LIKE :prefix || '%'")
    suspend fun byDatePrefixOnce(prefix: String): List<InsuranceEntity>
    @Query("SELECT * FROM insurance ORDER BY expiryDate")
    suspend fun allOnce(): List<InsuranceEntity>
    @Query("SELECT * FROM insurance WHERE id = :id")
    suspend fun byId(id: String): InsuranceEntity?
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(item: InsuranceEntity)
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertAll(items: List<InsuranceEntity>)
    @Update suspend fun update(item: InsuranceEntity)
    @Query("DELETE FROM insurance WHERE id = :id") suspend fun deleteById(id: String)
    @Query("DELETE FROM insurance") suspend fun clear()
}
